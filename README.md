...
> **Figure 1.** **A:** Top of a typical MultiQC report. The general statistics table
> can be seen with metrics gathered across multiple analysis outputs for each sample.
> **B:** Example of an overlay plot. GC content data from FastQC is overlayed, with
> datasets coloured by FastQC status. 
> **C:** Sample highlighting shown with coverage decay plots from Qualimap. The MultiQC
...
duplication scores, along with other key metrics (Fig 1A). Groups of
...
identified across metrics (Fig 1C). For example, we have plotted insert sizes measured on the
...
reducing time spent on QC and the chance of manual error.
...
RNA depletion effeciences, WG sequencing insert sizes and so on). MultiQC can print
